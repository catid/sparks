...
| --- | --- |
| Cerberus node 1 (`cerberus1`) | DSpark supervisor, TP rank 0/API, TP2-edge readiness, dashboard, optional Nginx |
| Cerberus node 2 (`cerberus2`) | TP rank 1 container controlled from C1; no autonomous model supervisor |
| Cerberus node 3 (`cerberus3`) | physical ring member, independent rack telemetry/kiosk and Audio8 TTS; no rank in the active DSpark service |
...
| `cerberus3-audio8.service` | C3 | `systemd/cerberus3-audio8.service.in` | isolated Audio8 API; intended active/enabled, with no playback loop |
...
| --- | --- | --- | --- |
| `/etc/hostname` and `/etc/hosts` | all three | `scripts/install-cluster-host-identity.sh` plus `hosts/cerberus*.hosts` | canonical short name; loopback only, no DHCP pins |
| NetworkManager `cerberus-mgmt` profile | all three | `scripts/install-cerberus-management.sh` | DHCP on `enP7s7` with explicit `cerberusN` client hostname |
| `/etc/avahi/avahi-daemon.conf` | all three | `scripts/install-cerberus-mdns.sh` | publishes canonical `cerberusN.local` on `enP7s7` only |
| `/etc/netplan/40-cx7.yaml` | all three | corresponding `netplan/cerberus*-40-cx7.yaml` | exact ring-edge configuration |
...
| `/etc/default/cerberus3-audio8` | C3 | optional local file | mode 0600; may name the private approved-reference directory |
| `/etc/nginx/sites-available/dgx-spark-dashboard` | Spark 1 | `dashboard/nginx-cerberus1-dashboard.conf` | enabled by symlink in `sites-enabled` |
| `/etc/nginx/ssl/cerberus1.lan.crt` | C1 | generated by `bin/install-dashboard-web.sh` | self-signed certificate for `cerberus1.lan` and `cerberus1.local` by default |
| `/etc/nginx/ssl/cerberus1.lan.key` | C1 | generated locally | default private key; never commit or copy into docs |
...
sudo cmp --silent netplan/cerberus1-40-cx7.yaml /etc/netplan/40-cx7.yaml \

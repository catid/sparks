...
RAM on either Cerberus node. In this placement the collector:

- samples Cerberus 1 and Cerberus 2 through separate, read-only SSH probes;
- scrapes the single TP2 vLLM metrics endpoint on Cerberus 1;
...
The existing Cerberus 1 placement remains the default. Setting
`SPARK1_SSH_HOST` opts into remote Cerberus 1 collection; leaving it unset keeps
the direct local probe.

The `SPARK1_*` and `SPARK2_*` names are intentionally retained as compatible
configuration variables. They refer to Cerberus 1 and Cerberus 2; all node
names emitted by `/api/status` and shown in the UI are canonical Cerberus
identities.
...
The recommended SSH authorization on **each Cerberus node** is a key forced to
the fixed probe:
...
Install the fixed command on each Cerberus node:
...
The vLLM metrics scrape uses Cerberus 1's normal HTTP endpoint. Keep it on the
...
```ini
SPARK1_SSH_HOST=monitor@cerberus1.local
SPARK2_SSH_HOST=monitor@cerberus2.local
SPARK1_VLLM_URL=http://cerberus1.local:8889
...
sudo -u dgx-dashboard ssh \
  ...
  monitor@cerberus1.local ignored
...
sudo -u dgx-dashboard ssh \
  ...
  -o StrictHostKeyChecking=yes \
  -o UserKnownHostsFile=/home/dgx-dashboard/.ssh/known_hosts \
  monitor@cerberus2.local ignored
```
...
flag removes the Cerberus 1 hostname guard and validates all remote-collector
settings:
...
sudo env \
  SPARK_SERVICE_USER=dgx-dashboard \
  DASHBOARD_ENV_FILE=/etc/dgx-spark-dashboard/dashboard.env \
  DASHBOARD_WEB_HOST=cerberus-dashboard.lan \
...
curl -fsS --user "${dashboard_auth}" \
  --cacert /etc/nginx/ssl/cerberus-dashboard.lan.crt \
  https://cerberus-dashboard.lan/api/status
...
Do not disable Cerberus 1's collector until the third-host service is verified.
Then move the dashboard DNS name/reverse proxy to the third host and stop only
the old dashboard unit:

```bash
# Run on Cerberus 1 after the remote dashboard is proven.
...
back, re-enable the same unit on Cerberus 1; because `SPARK1_SSH_HOST` is absent
from the normal Cerberus 1 environment, it resumes local collection.

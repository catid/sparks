...
cleanup_profiles() {
  rm -f -- "${rendered_path}" "${invalid_path}" "${root}/.mia3-sync-root"
}
...
MIA3_PROFILE_NAME="${rendered_name}" \
  ...
  CEREBRUS1_MGMT_IP=172.20.30.1 \
  CEREBRUS2_MGMT_IP=172.20.30.2 \
  CEREBRUS3_MGMT_IP=172.20.30.3 \
...
MIA3_ENV_FILE="${rendered_name}" bash -c '
  ...
  [[ "${MIA3_ENV_BASENAME}" == "$2" ]]
  [[ "${HEAD_MGMT_IP}" == 172.20.30.1 ]]
  [[ "${RANK1_MGMT_IP}" == 172.20.30.2 ]]
  [[ "${RANK2_MGMT_IP}" == 172.20.30.3 ]]
  [[ "${MASTER_ADDR}" == 172.20.30.1 ]]

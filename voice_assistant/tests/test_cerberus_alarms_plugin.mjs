...
import plugin, {
  createAlarmTools,
  requestAlarmService,
} from "../openclaw/plugins/cerberus-alarms/index.js";
...
test("rejects overlong service identifiers instead of truncating them", async () => {
  const tools = createAlarmTools(async () => ({
    alarm: {
      id: "0123456789abX",
      kind: "timer",
      label: null,
      ...
      local_due_at: "2026-08-12T07:00:00-05:00",
      timezone: "America/Chicago",
    },
  }));
  await assert.rejects(
    tools.find((tool) => tool.name === "timer_set").execute("call", {
      duration_seconds: 1,
    }),
    /incomplete event/,
  );
...
function mockRequest(onEnd, onDestroy = () => {}) {
  const request = new EventEmitter();
  request.setTimeout = () => request;
  request.write = () => true;
  request.end = () => onEnd(request);
  request.destroy = () => onDestroy(request);
  return request;
}

test("enforces one total deadline across a trickled response", async () => {
  let interval;
  let destroyCount = 0;
  const requestImpl = (_options, onResponse) => mockRequest(
    () => {
      const response = new EventEmitter();
      response.statusCode = 200;
      onResponse(response);
      const bytes = Buffer.from('{"alarms":[]}');
      let index = 0;
      interval = setInterval(() => {
        response.emit("data", bytes.subarray(index, index + 1));
        index += 1;
  ...
  await assert.rejects(
    requestAlarmService("/v1/alarms", { requestImpl, timeoutMs: 35 }),
    /timed out/,
  );
  assert.equal(destroyCount, 1);
});

test("rejects aborted, errored, and prematurely closed responses", async (context) => {
  for (const scenario of ["aborted", "error", "close"]) {
    await context.test(scenario, async () => {
      ...
      const requestImpl = (_options, onResponse) => mockRequest(
        () => {
          ...
          response.statusCode = 200;
          onResponse(response);
          response.emit("data", Buffer.from('{"alarms":'));
          if (scenario === "error") response.emit("error", new Error("private transport detail"));
          else response.emit(scenario);
          response.emit("close");
        },
        () => { destroyCount += 1; },
      );
      ...
      await assert.rejects(
        requestAlarmService("/v1/alarms", { requestImpl, timeoutMs: 100 }),
        (error) => {
          assert.match(
            error.message,
            scenario === "error" ? /response failed/ : /ended prematurely/,
          );
